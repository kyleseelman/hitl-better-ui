...
TASK_FILENAME = '../eval_3_data/ir-p3-hitl-performer-tasks.json'
# ENTITY_FILENAME_TEMPLATE = '../demo-data/{}/entity.txt'
ENTITY_FILENAME_TEMPLATE = '../eval_3_data/{}/entity.txt'
# TERM_FILENAME_TEMPLATE = '../demo-data/{}/terms.tsv'
TERM_FILENAME_TEMPLATE = '../eval_3_data/{}/terms.tsv'
# LABEL_FILENAME_TEMPLATE = '../demo-data/{}/doc_{}.tsv'
LABEL_FILENAME_TEMPLATE = '../eval_3_data/{}/doc_{}.tsv'
#vRESULTS_JSON_FILENAME = 'data/outputs_to_IE_fullHITL.json'
RESULTS_JSON_FILENAME = '../eval_3_data/outputToHITL.json'
# RESULTS_JSON_FILENAME = '../demo-data/dry-run-topics.final.json'
#DATA_DIR = '../demo-data'
DATA_DIR = '../eval_3_data'
...
UNLABELED_ENT_TEMPLATE = """
<button id="unlabeled" name="ent" type="button" class="btn btn-outline-dark btn-plain" value="{text}">{text}</button>
"""
LABELED_ENT_TEMPLATE = """
<button id="labeled" name="ent" type="button" class="btn btn-outline-dark btn-plain active" value="{text}">{text}</button>
"""
...
def topic():
    ...
    for task in tasks:
        if req_id.startswith(task['task-num']):
            ...
            count = 0
            idx = 0
            # If using ETM the doc numbers are nested in arrays so have to do nested
            for doc_id in doc_numbers:
                #print(id_num)
                doc_id = int(doc_id)
                #for doc_id in id_num:
                snippet = ''
                label_filename = LABEL_FILENAME_TEMPLATE.format(req_id, doc_id)
                with open(label_filename, 'r') as f:
                    next(f)
...
def docu():
    ...
    al_flag = request.args.get('al_flag')
    anno_flag = request.args.get('anno_flag')
    form_array = request.args.get('array')
    print(form_array)
    print("Anno flag: ", anno_flag)
    dir = '../eval_3_data/{}'.format(req_id)
    MAX_DOCS = len([name for name in os.listdir(dir) if os.path.isfile(os.path.join(dir, name))])
    print(MAX_DOCS)
    #if request.is_ajax() and request.method == 'GET':
...
# SAVE ANSWERS FROM ANNOTATION
    if form_array:

        test_split = re.split('=on', form_array.replace("&",""))
        print(test_split)
        doc_id = int(re.split("_", test_split[0])[1])
        print(doc_id)
        ...
        with open(label_filename, 'w') as f:
            ...
            for req_id, task_label, req_label, sent_id, text in rows:
                print(sent_id)
                if 'req_' + sent_id in test_split:
                    req_label = 'checked'
                    task_label = 'checked'
                else:
                    req_label = ''
                    if 'task_' + sent_id in test_split:
                        task_label = 'checked'
                    else:
                        task_label = ''
...
def update():
    ...
    print(topics)
    topics[int(num)-1] = topic
    #print(topics)

    print(old_topics)
    print(topics)
    old_topics[int(num)-1] = topics[int(num)-1]
    #print(topic)
    topics = old_topics
    print(topics)
    np.save("./topics.npy", topics)
